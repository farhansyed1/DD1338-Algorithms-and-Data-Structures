...
public class Box implements Comparable<Box> {
    ...
    //Exercise 1
    /**
     * Compares the volume of a box with another box object
     * @param anotherBox the box to be compared with
     * @return           an integer value depending on whether the other box is smaller, larger or equal in volume
     */
    @Override
    public int compareTo(Box anotherBox) {
        int thisVolume = this.volume();
        int anotherVolume = anotherBox.volume();
        if (thisVolume < anotherVolume) {
            return -1;
        }
        else if (thisVolume == anotherVolume) {
            return 0;
        }
        else {
            return 1;
